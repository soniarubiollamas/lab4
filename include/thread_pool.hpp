...
class thread_pool
{
  ...
  void worker_thread() //  continuously pops tasks from the queue and executes them while the _done flag is false.
  {
    while (!_done) // while the thread pool is not done
    {
      ...
      std::cout << " executed a task" << std::endl;
      }
      else std::this_thread::yield(); // yield to the OS (the thread does not have work to do)
    }
  }
...
public:
  ...
  void wait()
  {
    while (!_work_queue.empty()) // while the queue is not empty
